...
export class SpanBarn {
    ...
    if (this.config.beforeSend) {
      ...
      if (!finalSpan) {
        if (this.config.debug) {
          console.debug(
            "[SpanBarn] Span dropped by beforeSend filter:",
            span.name,
          );
        }
        return;
  ...
  async flush(): Promise<void> {
    ...
    if (Date.now() < this.backoffUntil) {
      if (this.config.debug) {
        console.debug(
          `[SpanBarn] Skipping flush, backing off for ${Math.round((this.backoffUntil - Date.now()) / 1000)}s`,
        );
      }
      return;
    }
    ...
    if (!success) {
      ...
      const delay = Math.min(
        5000 * 2 ** (this.consecutiveFailures - 1),
        5 * 60 * 1000,
      );
